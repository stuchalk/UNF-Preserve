...
<div class="right">
	...
	<?php echo $this->element('preserve/stockphoto',['species'=>$invert,'site'=>$site]); ?>
	
	<!-- Display species data -->
	<div id="topright" style="float: left;">
		<h2>Invertebrate Inventory</h2>
		<h3><i><?php echo $invert['sname']; ?></i></h3>
		<p><b>Common Name:</b> <?php echo $invert['cname']; ?><br />
		<b>Order:</b> <?php echo $invert['order']; ?><br />
		<b>Family:</b> <?php echo $invert['family']; ?><br />
		<b>Last Updated:</b> <?php echo date("F j, Y, g:i a",strtotime($invert['updated'])); ?>
		<?php if($invert['comment']!="") { echo "<br /><b>Comments:</b> ".$invert['comment']; } ?>
		</p>
		<?php
			if($invert['url']!="")
			{
		...
		<?php
			$script="setProtection('".$invert['id']."',this.value)";
			echo "<div>".$this->Form->input('protected',['type'=>'radio','value'=>$invert['protected'],'div'=>false,'options'=>['0'=>'Open','1'=>'Protected'],'legend'=>false,'onchange'=>$script])."</div>";
		?>
	</div>
	<div class="clear"></div>
	...
	<div class='clear'></div>
	<p>&nbsp;</p>
	<?php echo $this->element('preserve/addphotoform',['type'=>'invert','id'=>$invert['id'],'pcount'=>$pcount,'col'=>$col]); ?>
</div>