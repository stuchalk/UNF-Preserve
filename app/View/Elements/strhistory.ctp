...
$history=$this->requestAction('/datastreams/history/'.$args['pid'].'/'.$args['dsid']);
?>
...
<div class="leftdiv">
	...
	<div id="strhistory" class="leftdivbody" style="<?php echo ($vis==true? 'display: block;':'display: none;'); ?>">
		<ul>
			...
			{
				echo "<li>".$this->Html->link(date("m/d/y, g:i:s a",strtotime($date)),'/datastreams/view/'.$args['pid'].'/'.$args['dsid'].'?asOfDateTime='.$date)."</li>";
			}
			?>
		</ul>
	</div>
